...
namespace Lib {
	...
	struct ImmutableListMap {
		...
		ImmutableListMap<K, V> append_head(const std::shared_ptr<std::pair<K, V>>& key_value) const;
		ImmutableListMap<K, V> tail() const;
		ConstNodeIter find(const K& key) const;
		std::pair<ConstNodeIter, ImmutableListMap<K, V>> erased(const K& key) const;
